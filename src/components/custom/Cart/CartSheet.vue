...
<template>
  <Sheet v-model:open="open">
    <SheetTrigger>
      ...
      <span
        class="absolute top-1 right-2.5 bg-red-500 text-white rounded-full text-xs p-1 w-5 h-5 flex items-center justify-center"
        >{{ cartCount }}</span
      >
    </SheetTrigger>
    <SheetContent class="flex flex-col max-h-screen overflow-hidden">
      <SheetHeader>
        ...
        <div v-if="cartCount === 0" class="flex flex-col items-center justify-center py-6">
          ...
          <button
            ...
            "
            class="bg-safariGreen p-2 rounded-md hover:bg-safariGreenHover transition duration-300 text-white font-bold mt-2"
          >
            View Products
          </button>
        </div>
        <div v-else>
          <div v-for="item in cartList">
            <div class="md:flex relative md:my-1 -my-5 w-full">
              <button
                @click="() => navigateToProduct(item._id)"
                :key="item._id"
                class="bg-gray-200 dark:bg-gray-800 dark:text-gray-200 p-2 rounded-md flex items-center justify-start cursor-pointer hover:bg-gray-100 transition duration-300 w-full"
              >
                <img
                  width="{100}"
                  height="{100}"
                  :src="item.image"
                  alt="item.product"
                  class="w-16 md:w-20 rounded-md m-2 mr-4"
                />
              ...
              <button @click="() => removeHandler(item)" aria-label="Open Cart">
                <OhVueIcon
                  name="io-close"
                  class="absolute top-0 right-0 z-40 hover:text-white transition duration-300 bg-white rounded-full w-6 h-6 p-1 text-red-500 hover:bg-red-500"
                />
              </button>
            </div>
          </div>
      ...
      <div class="absolute bottom-0 right-0 w-full p-4 bg-background flex flex-col items-end">
        ...
        <button
          ...
          class="bg-safariGreen hover:bg-safariGreenHover transition duration-300 text-white p-2 rounded-md mt-2 px-4"
          @click="
            () => {
              open = false;
              router.push('/cart');
