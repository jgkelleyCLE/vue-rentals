...
<template>
  <div class="p-2 w-full h-full bg-gray-200 dark:bg-gray-700 rounded-md">
    ...
    <div v-for="item in searches?.slice(0, 10)" :key="item._id">
      <RouterLink
        :to="`/admin/search-details/${item._id}`"
        :key="item._id"
        class="flex items-center justify-between gap-2 bg-white dark:bg-gray-800 p-2 rounded-md my-1 cursor-pointer hover:bg-white/60 dark:hover:bg-gray-900 transition duration-300"
      >
        <h1>"{{ item.term }}"</h1>
