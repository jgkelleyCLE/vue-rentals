 ...
 <Sheet v-model:open="open">
    ...
    <div class="relative inline-block">
        ...
        <OhVueIcon 
            ...
            class="absolute top-9 right-1 w-5 h-5 z-20 opacity-75" 
        />
    </div>
</div>
        ...
        <div v-for="item in mobileMenuLinks" :key="item.id">
            ...
            <button aria-label="`Navigate to ${item.title}`" key={item.id} class="relative w-full" @click="()=> navigationHandler(item)">
        ...
        <!-- LOCATION BUTTON -->
        <button aria-label="Navigate to Location Page" class="relative w-full" @click="()=> {
            open = false
...
<!-- GALLERY BUTTON -->
        <button aria-label="Navigate to Gallery Page" class="relative w-full" @click="()=> {
            open = false
            router.push('/gallery')}">
  <div class="bg-black/30 inset-0 rounded-md absolute z-20 top-0 left-0 w-full"></div>
