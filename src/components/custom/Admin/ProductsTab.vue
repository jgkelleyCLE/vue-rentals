...
<template>
    ...
    <TableBody>
      ...
      <TableRow v-else v-for="item in products" :key="item._id" :class="[
          'border-2 border-gray-300 relative',
          { 'bg-red-100 dark:bg-red-950/75': item.status === 'Inactive' }
        ]">
        <TableCell class="hidden md:table-cell"><RouterLink :to="`/product/${item._id}`"><img width="75" height="50" :src="item.image" :alt="item.product" /></RouterLink></TableCell>
              <TableCell><RouterLink :to="`/product/${item._id}`">{{item.product}}</RouterLink></TableCell>
