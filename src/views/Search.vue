...
const { data: searchData, isLoading, isError, error } = useQuery({
  ...
  queryFn: async () => {
    const response = await fetch(
      `https://tentlify-ecom.up.railway.app/api/products/search?q=${encodeURIComponent(searchQuery.value)}`
    );
...
<template>
  <div class="mt-20 mx-auto w-11/12">
    ...
    <div class="flex flex-col items-center w-11/12 mx-auto">
      ...
      <!-- RESULTS -->
      <div
        v-else-if="searchData && searchData.length > 0"
      ...
      <div v-else class="p-8 text-center">
        ...
        <RouterLink
          :to="`/product`"
          class="bg-safariGreen hover:bg-safariGreenHover text-white font-bold p-2 transition duration-300 rounded-md"
        >
          Browse all products
        </RouterLink>
      </div>
