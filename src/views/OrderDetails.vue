...
<template>
    ...
    <h1  class="text-2xl ml-8">{{order?.title}} Order Details</h1>
            <div class="flex flex-col items-start ml-8">
                ...
                <p class="text-sm italic text-gray-500 mb-1">Event Date: {{new Date(order?.eventDate).toLocaleDateString()}}</p>

                <OrderStatusSelect :item="order" />
            </div>

            
            <div  v-for="item in order?.orderItems" :key="item._id" class="flex flex-row items-center justify-between border-b border-gray-200 p-4 w-11/12 mx-auto">
