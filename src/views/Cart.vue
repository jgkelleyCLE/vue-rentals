        ...
        <div class="w-[95%] md:w-[65%] mx-4" >
            ...
            <div class="flex items-center justify-between p-2 shadow-md shadow-black/20 dark:bg-gray-800 bg-gray-100 w-full rounded-md my-2 relative" v-for="item in cartList">
                <div class="flex items-start ">
                    <img width={100} height={50} class="w-32 object-fit rounded-md cursor-pointer mr-2" :src="item.image" :alt="item.product" @click="()=> router.push(`/product/${item._id}`)" />
  
                    <div class="flex flex-col items-start">
                      <h1 class="text-xl font-bold cursor-pointer" @click="()=> router.push(`/product/${item._id}`)">{{item.product}}</h1>
                      ...
                      <button aria-label="decrease quantity" @click="()=> decreaseQuantity(item)"><OhVueIcon name="co-minus" /></button>
                      <h1 class="w-[50px] border-2 border-gray-400 px-2 rounded-md">{{item.cartQuantity}}</h1>
                      
                      <button aria-label="increase quantity" @click="()=> increaseQuantity(item)"><OhVueIcon name="co-plus" /></button>
                      
                    </div>
                    
                    </div>
                    <button @click="()=> removeHandler(item)" aria-label="`Remove product from cart`" ><OhVueIcon name="io-close" class=" absolute -top-1 -right-1 z-40 hover:text-white transition duration-300 dark:bg-white bg-red-500 rounded-full w-6 h-6 p-1 dark:text-red-500 text-white dark:hover:bg-red-500 hover:bg-red-500 dark:hover:text-white" />
                    </button>
            ...
            <div class="flex justify-end">
                <button aria-label="Remove all items from cart" class="bg-safariGreen hover:bg-safariGreenHover p-2 px-6 rounded-md text-white transition duration-300 font-bold mt-2 mb-2" @click="()=> cartStore.clearCart()">Clear Cart</button>
            </div>
