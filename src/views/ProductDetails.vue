    ...
    <div v-else class="flex gap-2 mt-2 flex-col lg:flex-row items-start">
      ...
      <div class="lg:w-1/2 w-full">
        <img
          width="{1000}"
          height="{600}"
          class="w-full rounded-md max-h-[700px] object-contain"
          :src="product?.image"
          alt="product?.product"
        />
      </div>
      ...
      <div class="flex flex-col items-start p-4 rounded-md bg-gray-200 dark:bg-gray-600 max-h-[600px] lg:w-1/2 w-full">
        ...
        <h1 class="text-gray-600 dark:text-gray-300 font-semibold my-4">
          ${{ product?.price.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) }}
        </h1>

        <input
          type="number"
          class="w-full max-w-[300px] p-2 text-black font-bold text-xl rounded-md dark:bg-gray-400"
          placeholder="Qty"
          v-model="quantity"
        />
        <button
          aria-label="`Add product to cart`"
          class="w-full max-w-[300px] bg-green-400 hover:bg-green-500 font-bold p-2 rounded-md mt-2 transition duration-300"
